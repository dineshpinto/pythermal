...
__version__ = "v1.3.0"
__all__ = ['System', 'check_lattice', 'main']
...
class System:
    def __init__(self, initial_value, option, lattice_a=None, lattice_b=None):
        ...
        if self.manual_lattice:
            ...
            a = mt.sqrt(self.nol_a) * self.nsa
            b = (self.nsa - mt.sqrt(self.nol_a))
            self.link_pos = a - b
...
def main(initial_values, options, lattice_a=None, lattice_b=None):
    ...
    # routine1.py (Eigenstates, Hamiltonian, Eigenvalues and Eigenvectors)

    # Eigenstates
    if s.manual_lattice:
        eigenstates, nos = routine1.eigenstates_lattice(s.lat, s.nop)
        eigenstates_a, nos_a = routine1.eigenstates_lattice(lattice_a,
                                                            s.nop)
    else:
        eigenstates, nos = routine1.eigenstates_lattice(s.lat, s.nop,
                                                        s.lat_del_pos)
        eigenstates_a, nos_a = routine1.eigenstates_lattice(s.lat, s.nop,
                                                            s.lat_del_pos_a)
    ...
    if existence[0]:
        ...
        print('Hamiltonian...')
        hamiltonian = routine1.parallel_call_hamiltonian(eigenstates, nos,
                                                         s.nsa, s.nop)
    ...
    if existence[1]:
        ...
        hamiltonian_a = \
            routine1.parallel_call_hamiltonian(eigenstates_a, nos_a,
                                               s.nsa, s.nop)
    ...
    if existence[2] and existence[3]:
        ...
        print('Diagonalizing...')
        eigenvalues, eigenvectors = routine1.eigenvalvec(hamiltonian)
    ...
    if existence[4] and existence[5]:
        ...
        eigenvalues_a, eigenvectors_a = routine1.eigenvalvec(hamiltonian_a)
    ...
    # ------routine2.py (State Relabelling and Initial State)-----

    # State Relabelling
    r_time1 = time.time()

    if s.manual_lattice:
        re_states = routine2.relabel(eigenstates, s.nop, s.nol_b,
    ...
    if s.eigenvector_ab:
        ...
        psi_initial = routine2.init_state(eigenvectors_a, re_states, nos,
                                          s.nop, s.initial_state)

    # -----routine3.py (Time Evolution and Von-Neumann Entropy)-----
    ...
    if existence[6] and existence[7] and existence[8]:
        ...
        avg_particles_a = output.read_file(path_td, names[7])
        avg_particles_b = output.read_file(path_td, names[8])
    else:
        print('Time evolving...')
        psi_t = routine3.time_evolution(psi_initial, hamiltonian, nos,
                                        s.timesteps)
        avg_particles_a, avg_particles_b = \
            routine3.particle_counter(psi_t, s.timesteps, re_states, s.nop)

        output.write_file(path_td, names[6], psi_t)
        output.write_file(path_td, names[7], avg_particles_a)
        output.write_file(path_td, names[8], avg_particles_b)
    ...
    if existence[9] and existence[10]:
        ...
        vn_entropy_b, vn_trace2_b = \
            routine3.von_neumann_b(psi_t, re_states, nos, s.nol_b, s.nop)
    ...
    output.plot(x, avg_particles_a, titles[2], y_labels[2], x_labels[0],
                y_limits[2], path_td, image_name[2], chk)
    output.plot(x, avg_particles_b, titles[3], y_labels[3], x_labels[0],
                y_limits[2], path_td, image_name[3], chk)
    output.plot(x, avg_particles_b + avg_particles_a, titles[4], y_labels[4],
